...
def outpuGfx(text):
    ...
    global TAG
    pngpath = "gfx/interface/decisions_picture/" + TAG
    txtfile = open(interfacePath + "decisions_picture_" + TAG + ".gfx", "w", encoding="UTF-8")
    # spriteTypes = {
...
pngpathfirst = "..\gfx\interface\decisions_picture\\"
taglist = getDir(pngpathfirst)
print("将生成以下tag民族精神的gfx文件")
print(taglist)
input("")
interfacePath = "..\interface\decisions_picture\\"
if not os.path.exists(interfacePath):
    os.makedirs(interfacePath)
for TAG in taglist:
    pngpath = pngpathfirst + TAG
    gfxfilename = interfacePath + "decisions_picture_" + TAG + ".gfx"
