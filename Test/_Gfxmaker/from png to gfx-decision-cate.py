...
def outpuGfx(text):
    ...
    global TAG
    pngpath = "gfx/interface/decisions_category/" + TAG
    txtfile = open(interfacePath + "decisions_category_" + TAG + ".gfx", "w", encoding="UTF-8")
    # spriteTypes = {
    ...
    for txt in text:
        ...
        pngfullgfxname = "\t\tname = GFX_decision_category_" + txt + "\n"
...
pngpathfirst = "..\gfx\interface\decisions_category\\"
taglist = getDir(pngpathfirst)
print("将生成以下tag民族精神的gfx文件")
print(taglist)
input("")
interfacePath = "..\interface\decisions_category\\"
if not os.path.exists(interfacePath):
    os.makedirs(interfacePath)
for TAG in taglist:
    pngpath = pngpathfirst + TAG
    gfxfilename = interfacePath + "decisions_category_" + TAG + ".gfx"
